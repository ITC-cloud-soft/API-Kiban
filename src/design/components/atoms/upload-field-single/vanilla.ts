...
export class UploadFieldSingle extends VanillaComponent<HTMLDivElement> {
  ...
  constructor({ element }: VanillaComponentConstructor<HTMLDivElement>) {
    super({ element });
    const inputEl = element.querySelector<HTMLInputElement>('.a-upload-field-single__input');

    if (!inputEl) return;

    const uploadFileListEl = element.querySelector<HTMLDivElement>('.a-upload-field-single__file-list');

    const clearImageButton = element.querySelector<HTMLButtonElement>('.a-upload-field-single__reset');
    clearImageButton?.addEventListener('click', () => {
      const arrayImagePreview = document.querySelector('.a-upload-field-single__preview');
      ...
      this.element.classList.remove('a-upload-field-single--with-reset');
      inputEl.value = '';
    });

    const updateUpdateFileListClassName = () => {
      if (uploadFileListEl) {
        uploadFileListEl.classList.toggle('a-upload-field-single__file-list--no-child', this.arrayImage.length === 0);
      }
    };
    ...
    const handleClearButtonClick = (index: number) => {
      const arrayImagePreview = element.querySelector('.a-upload-field-single__preview');
      const arrayImagePreviewCustom = document.getElementsByClassName("customer-li");
      const newArrayImage = this.arrayImage.filter(image => image != this.arrayImage[index]);
      // const dataTransfer = new DataTransfer();
      const dataTransfer = this.getDataTransfer();

      if (arrayImagePreview) {
        if (arrayImagePreviewCustom.length > 0) {
          arrayImagePreview.removeChild(arrayImagePreview.childNodes[arrayImagePreview.childElementCount + index - 1]);
        } else {
          arrayImagePreview.parentNode?.removeChild(arrayImagePreview);
        }
      }
    ...
    const createNewImageNode = (index: number) => {
      ...
      li.className = 'a-upload-field-single__image';
      ...
      if (dt) {
        ...
        button.className = 'a-upload-field-single__clear-button';
    ...
    const handleInputChange = (target: HTMLInputElement, e: Event) => {
      if (target.files && target.files.length > 0) {
        ...
        const previewHolder = element.querySelector<HTMLDivElement>('.a-upload-field-single__preview-holder');
        const dt = this.getDataTransfer();
        if (!dt) {
          this.element.classList.toggle('a-upload-field-single--with-reset', this.arrayImage.length > 0);
        }
        if (inputEl.accept.startsWith('image/', 0)) {
          let ul = element.querySelector<HTMLUListElement>('.a-upload-field-single__preview');

          if (!ul) {
            ul = document.createElement('ul');
            ul.className = 'a-upload-field-single__preview';
          }
          ...
          for (let index = amountCurrentImage; index < amountCurrentImage + arrayNewImage.length; index++) {
            ...
            if (childEl) {
              ...
              const listItem = new UploadListItem({ element: newFileListItemEl } as VanillaComponentConstructor<
                HTMLLIElement
              >);
              if (dt) {
...
export function autoInitUploadFieldSingle() {
  document.querySelectorAll<HTMLDivElement>('.a-upload-field-single').forEach(element => new UploadFieldSingle({ element }));
}
