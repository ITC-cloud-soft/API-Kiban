...
const config: ConfigurationFactory = async () => {
  const vanillaExamplePath = '../src/ss-html/';
  ...
  const buildPlugins = fs
    ...
    .map(sourceFileName => {
      ...
      return new HtmlWebpackPlugin({
        ...
        inject: 'body',
        minify: false, // Because `src/ss-html/render` beautify the HTML
  ...
  return {
    ...
    output: {
      path: resolve(__dirname, '../dist/ss-html'),
    ...
    devServer: {
      ...
      port: 8000,
      open: true,
      contentBase: resolve(__dirname, '../dist/ss-html'),
