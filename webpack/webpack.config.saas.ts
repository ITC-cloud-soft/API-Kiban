...
const config: ConfigurationFactory = async () => {
  const vanillaExamplePath = '../src/saas/';
  ...
  const buildPlugins = fs
    ...
    .map(sourceFileName => {
      ...
      return new HtmlWebpackPlugin({
        ...
        inject: 'body',
        minify: false, // Because `src/saas/render` beautify the HTML
  ...
  return {
    ...
    output: {
      path: resolve(__dirname, '../dist/saas'),
    ...
    devServer: {
      ...
      port: 8000,
      open: true,
      contentBase: resolve(__dirname, '../dist/saas'),
